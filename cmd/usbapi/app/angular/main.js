...
app.controller('DashboardController', function($scope, $rootScope, $http) {
  ...
  this.startNewDevice = function(patientId) {
  	console.log($scope.pid, this.dev_bus, this.dev_addr);
  	this.state = 0;
  	$.snackbar({content: "start monitor [" + $scope.pid + "] with device bus-address of [" + this.dev_bus + ":" + this.dev_addr + "]"});
  };
});
...
app.controller('PatientViewController', function($rootScope, $scope, $routeParams, $http, $interval){
	...
	$scope.refreshGraph = function() {
		...
		if (mnt_type == 'ecg') {
			...
			$scope.params.limits = 6*$scope.params.rate;
			$scope.params.delay = 100; // milliseconds
			$scope.params.sample_delay = 1000 * $scope.params.rate / $scope.params.delay;
		...
		.success(function(data) {
			...
			dps_max = dps_max || (d[0][ch] * param.gain + param.min);
			dps_min = dps_min || (d[0][ch] * param.gain + param.min);
			...
			//chart.options.axisY.minimum = dps_min - 1;
			//chart.options.axisY.maximum = dps_max + 1;
		...
		var updateGraph = function() {
			...
			for (var i=0,len=$scope.params.sample_delay; i < len; i++)
			{
				if (buffer_dps.length > 0) {
					$scope["dps"].push(buffer_dps.shift());
